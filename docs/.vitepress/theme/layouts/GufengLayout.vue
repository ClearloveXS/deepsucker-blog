...
// ---------- 五观（云山四观+雪顶低谷） ----------
// 顺序按情感曲线排：登·起 → 壮 → 静求 → 悟（前卷顶点）→ 骤入低谷（雪顶）
const GUANS = [
  {
    no: '一觀', name: '天柱', img: '/images/gufeng/tianzhu.jpg',
    src: '李白《夜宿山寺》',
    poem: ['危樓高百尺', '手可摘星辰', '不敢高聲語', '恐驚天上人'],
    note: '開卷便是登。柱不是柱，是垂落的經幡——雲海以下是人間，雲海以上是問題；紅衣人站在中間，本身就是答案，可他未必知道自己已是答案。'
  },
  {
    no: '二觀', name: '赤峽', img: '/images/gufeng/chixia.jpg',
    src: '李白《望天門山》',
    poem: ['天門中斷楚江開', '碧水東流至此回', '兩岸青山相對出', '孤帆一片日邊來'],
    note: '樓在身後，山在眼前。整面峽谷是凝固的火焰，棧道是火裡唯一的退路——也是唯一的進路。白衣人沒有翅膀，衣袂起來的時候，比翅膀好看。'
  },
  {
    no: '三觀', name: '松風', img: '/images/gufeng/songfeng.jpg',
    src: '賈島《尋隱者不遇》',
    poem: ['松下問童子', '言師采藥去', '只在此山中', '雲深不知處'],
    note: '松問童子，童子問雲。青灰萬仞之間只許一點紅，這一點紅就是整幅畫的眼。松把風盤住，人把心放空；雲在腳下假裝海。'
  },
  {
    no: '四觀', name: '佛殿', img: '/images/gufeng/fodian.jpg',
    src: '王維《終南別業》',
    poem: ['興來每獨往', '勝事空自知', '行到水窮處', '坐看雲起時'],
    note: '須彌藏芥子，芥子納須彌。三個剪影朝聖的方向，也是光進來的方向——巨構再大，不過是給晨光畫的框。坐到這裡，前四觀算是登到頂了。可古人沒說頂後是什麼——下一觀要揭曉。'
  },
  {
    no: '五觀', name: '雪頂', img: '/images/gufeng/xueding.jpg',
    src: '柳宗元《江雪》',
    poem: ['千山鳥飛絕', '萬徑人蹤滅', '孤舟蓑笠翁', '獨釣寒江雪'],
    note: '上一觀還坐在晨光裡，下一觀鏡頭就壓到了地面。雪把一切聲音都吸收了，巨構只剩輪廓，人只剩動作——掃雪這個動作，一掃就是一生；可他掃的到底是雪，還是自己？'
  }
]

// ---------- 续四观（夜问·听雨·破迷·返苍 · 谷底漫长抬头） ----------
const XU_GUANS = [
  {
    no: '六觀', name: '夜航', img: '/images/gufeng/yemu.jpg', tag: '夜之意象',
    src: '蘇軾《水調歌頭》',
    poem: ['明月幾時有', '把酒問青天', '不知天上宮闕', '今夕是何年'],
    note: '雪裡埋久了，人會想抬頭。抬頭一看——月亮還在。不知天上宮闕，今夕是何年；可這一問，已經把人從雪裡拔出來了。燈是巨構的心跳，萬燈齊上時，整座樓閣開始呼吸。'
  },
  {
    no: '七觀', name: '雨幕', img: '/images/gufeng/yumu.jpg', tag: '雨之意象',
    src: '蘇軾《定風波》',
    poem: ['莫聽穿林打葉聲', '何妨吟嘯且徐行', '一蓑煙雨任平生'],
    note: '雪化了，就成了雨。雨是天地的簾子，把上一觀的孤絕隔在簾外——橋上僧人走得很慢，反正簾子這邊，沒有要趕的事。'
  },
  {
    no: '八觀', name: '長橋', img: '/images/gufeng/changqiao.jpg',
    src: '蘇軾《題西林壁》',
    poem: ['橫看成嶺側成峰', '遠近高低各不同', '不識廬山真面目', '只緣身在此山中'],
    note: '不識廬山真面目，只緣身在此山中——這是整卷最清醒的一句。橋沒有盡頭，盡頭只有雲。右側白衣人看了很久，他不是在看橋，是在看「路原來可以這樣走」。雲已散，山還在；路不同，景自不同。'
  },
  {
    no: '九觀', name: '沙海', img: '/images/gufeng/shahai.jpg', tag: '沙之意象',
    src: '王昌齡《從軍行》',
    poem: ['青海長雲暗雪山', '孤城遙望玉門關', '黃沙百戰穿金甲', '不破樓蘭終不還'],
    note: '城牆再長，長不過絲路；駝鈴再遠，遠不過斜陽。巨構與沙漠的關係，是互相成全的孤獨——可這一觀，孤獨不再悲涼。前四觀登天，第五觀入雪，後四觀抬首：樓塌了，駝鈴還在響；雲散了，斜陽還在燒。'
  }
]
...
// ---------- 诗卷（古人成句六首） ----------
const ZAYONG = [
  { title: '獨坐敬亭山', author: '李白', lines: ['眾鳥高飛盡', '孤雲獨去閑', '相看兩不厭', '只有敬亭山'] },
...
// ---------- 卷目 ----------
const TOC = [
  { id: 'guan-1', mark: '觀', name: '雲山四觀' },
  { id: 'guan-6', mark: '觀', name: '低谷歸程' },
  { id: 'jian-shi', mark: '史', name: '巨構簡史' },
  { id: 'banruo', mark: '觀', name: '行深般若' },
  { id: 'yong', mark: '詠', name: '詩卷' }
]
  ...
  <div ref="root" class="gf-page">
    ...
    <section class="gf-intro gf-reveal">
      ...
      <div class="gf-ci-wrap">
        <div class="gf-ci-head">開卷 · 王之渙《登鸛雀樓》</div>
        <div class="gf-poem gf-ci">
          <span v-for="line in OPEN_POEM.lines" :key="line" class="gf-poem-line gf-ci-line">{{ line }}</span>
        </div>
      </div>
    ...
    <section v-for="(g, idx) in GUANS" :key="g.no" :id="idx === 0 ? 'guan-1' : undefined" class="gf-guan">
      ...
      <figure class="gf-figure gf-reveal">
        ...
        <div class="gf-poem-on">
          <span v-for="line in g.poem" :key="line" class="gf-poem-line">{{ line }}</span>
          <span class="gf-poem-src">{{ g.src }}</span>
        </div>
      </figure>
      <p class="gf-note gf-reveal">{{ g.note }}</p>
    ...
    <section class="gf-xu-banner gf-reveal" id="guan-6">
      <div class="gf-rule" aria-hidden="true"><i></i><b>◆</b><i></i></div>
      <p class="gf-xu-text">
        前四觀從登天到入定，雲海一路相送；第五觀「雪頂」，鏡頭驟然壓低——千山鳥飛，萬徑人滅。雲山到這裡，被風雪整片掀走。<br />
        後四觀從夜問到沙歸，是一場漫長的抬頭：問月、聽雨、破迷、返蒼。雲散了，山還在。
      </p>
      <div class="gf-rule" aria-hidden="true"><i></i><b>◆</b><i></i></div>
    </section>
    ...
    <section class="gf-chapter gf-reveal" id="banruo">
      ...
      <blockquote class="gf-quote">
        不畏浮雲遮望眼，自緣身在最高層。
        <span class="gf-quote-from">——王安石《登飛來峰》</span>
      </blockquote>
    </section>

    <!-- ============ 咏 · 诗卷 ============ -->
    <section class="gf-chapter gf-chapter-wide gf-reveal" id="yong">
      <h2 class="gf-h2"><span class="gf-h2-no">詠</span>詩卷</h2>
      <div class="gf-prose">
        <p>巨構的孤與高，古人早已寫盡——登高、望月、聽鐘，六首皆是。</p>
      </div>
      <div class="gf-zayong">
        <div v-for="z in ZAYONG" :key="z.title" class="gf-zy-item">
          <div class="gf-zy-title">{{ z.title }}</div>
          <div class="gf-zy-author">{{ z.author }}</div>
...
@media (max-width: 860px) {
  ...
  .gf-poem-on .gf-poem-line:last-child { margin-bottom: 0; }
  .gf-poem-src {
    margin: 14px 0 0;
    padding-top: 0;
  }
